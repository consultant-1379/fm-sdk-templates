...
# Script to create fm-sdk-templates tar.gz for upload to nexus.
...
CHART_FILE="${PROJECT_DIR}/chart/eric-enmsg-custom-fm-oneflow/Chart.yaml"
BUILD_DIR="${PROJECT_DIR}/build"
BUILD_SOURCE_DIR="${BUILD_DIR}/fm-sdk-templates"
...
setup() {
  ...
  ${CP} -R ${CP_OPTS} ${PROJECT_DIR}/eric-enm-custom-models-fm-oneflow ${BUILD_SOURCE_DIR} || exit 1
}
...
create_tar() {
  local _version_=${1}
  local _tar_dest_=${2}
  _tar_name_="fm-sdk-templates-${_version_}.tar.gz"
  ${TAR} czvf ${_tar_dest_}/${_tar_name_} -C ${_tar_dest_} fm-sdk-templates
...
usage() {
  ...
  info "\t-g\t\tGenerate a new fm-sdk-templates tar file"
